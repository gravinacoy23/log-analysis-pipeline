...
def _create_files(
    ...
    """Creates the files for each service in the dedicated service path and opens it.
...
def _generator_loop(
    ...
    """Generates all the log lines for a given number of times and calls the corresponding auxiliary
        functions to genrerate format and write the log.
...
def generate_logs(iterations: int) -> None:
    """Orchestrates the log generation calling auxiliary functions to generate the data needed
        for the loop that generates the logs
